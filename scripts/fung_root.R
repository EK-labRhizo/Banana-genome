#Analysis of root fungal microbiome - rarefied----
library(tidyverse)
...
rootf_ASV_cts <- root_ASV_cts[rowSums(root_ASV_cts)>1000,]
...
iterations <- 2000        # Number of subsampling iterations
...
mean_subsampled_df%>%
  ...
  write_tsv("rare_fungRoot_cts.tsv")

#_clr transformation----
mean_subsampled_df <- read_tsv("rare_fungRoot_cts.tsv", lazy=FALSE)%>%
  tibble::column_to_rownames("seq_name")
...
Root_clr %>%
  tibble::rownames_to_column("seq_name")%>%
  write_tsv("rare_fungRoot_clr.tsv")

#PERMANOVA----
Root_clr <- read_tsv("rare_fungRoot_clr.tsv", lazy = FALSE)%>%
  tibble::column_to_rownames("seq_name")
...
adonis2(formula = Root_clr ~ plot_Row+plot_Column, data = sel_pos, method = "euclidean",by="term",strata = sel_pos$`A/B`)
adonis2(formula = Root_clr ~ plot_Row+plot_Column, data = sel_pos, method = "euclidean",by="term",strata = sel_pos$Genome)
...
write_tsv(PCA, 'fung_Root_clr_PCA.tsv')
loadings%>%
  tibble::rownames_to_column("ASV")%>%
  write_tsv('fung_Root_clr_PCA_load.tsv')

PCA <- read_tsv('fung_Root_clr_PCA.tsv', lazy = FALSE)
...
PCA%>%
  ...
  #facet_grid(time ~ .) +
  labs(title="Fungal communities of Banana Roots",
       x="PC 1 (13% variance)",
       y="PC 2 (6% variance)") +
  theme_dbox()+
...
ggsave("fung_Root_PCA12_treat.png", height = 180, width = 180, units = "mm", dpi=300)
...
ggsave("fung_Root_PCA23_treat.png", height = 180, width = 180, units = "mm", dpi=300)
...
PCA%>%
  ...
  #facet_grid(time ~ soil) +
  #facet_grid(time ~ .) +
  labs(title="fungal communities of Banana Root",
       x="PC 1 (13% variance)",
       y="PC 2 (6% variance)") +
  theme_dbox()+
  geom_hline(yintercept = 0, color = "black")+
...
ggsave("fung_Root_PCA12_AvB_genome.png", height = 180, width = 180, units = "mm", dpi=300)

#cultivar DA ASVs----
library(Maaslin2)
Root_clr <- as.data.frame(read_tsv("rare_fungRoot_clr.tsv", lazy = FALSE))
...
input_metadata <- filter(metadata, seq_name%in%names)%>%
  select(seq_name, Cultivar, Ext.Date)%>%
  as.data.frame()
...
fit_data = Maaslin2(input_data     = Root_clr, 
                    ...
                    plot_heatmap = FALSE,
                    plot_scatter = FALSE,
                    output         = "Root_Treat_blugg.E_fung", 
                    fixed_effects  = c("Cultivar"),
                    random_effects = c("Ext.Date"),
...
fit_data = Maaslin2(input_data     = Root_clr, 
                    ...
                    plot_heatmap = FALSE,
                    plot_scatter = FALSE,
                    output         = "Root_Prata.E_fung", 
                    fixed_effects  = c("Cultivar"),
                    random_effects = c("Ext.Date"),
                    reference      = c("Cultivar,Prata-Ana"))

#_list Cultivar DA bacteria----
DA_arab <- read_tsv('Root_arab.E_fung/significant_results.tsv', lazy = FALSE)%>%
  filter(qval <0.05)%>%
  mutate(ref="Pissang-Awak")
DA_calc4 <- read_tsv('Root_Calc4.E_fung/significant_results.tsv', lazy = FALSE)%>%
  filter(qval <0.05)%>%
  mutate(ref="Calcutta4")
DA_grand <- read_tsv('Root_grand.E_fung/significant_results.tsv', lazy = FALSE)%>%
  filter(qval <0.05)%>%
  mutate(ref="Grand-Naine-5/37")
DA_rose <- read_tsv('Root_rose.E_fung/significant_results.tsv', lazy = FALSE)%>%
  filter(qval <0.05)%>%
  mutate(ref="Aacv-Rose")
DA_blue <- read_tsv('Root_Treat_blue.E_fung/significant_results.tsv', lazy = FALSE)%>%
  filter(qval <0.05)%>%
  mutate(ref="Blue-banana")
DA_malac <- read_tsv('malac.E_fung/significant_results.tsv', lazy = FALSE)%>%
  filter(qval <0.05)%>%
  mutate(ref="M.Malaccensis")
DA_balb <- read_tsv('Root_balb.E_fung/significant_results.tsv', lazy = FALSE)%>%
  filter(qval <0.05)%>%
  mutate(ref="Balbisiana")
DA_natan <- read_tsv('Root_natan.E_fung/significant_results.tsv', lazy = FALSE)%>%
  filter(qval <0.05)%>%
  mutate(ref="Natan")
DA_apple <- read_tsv('Root_apple.E_fung/significant_results.tsv', lazy = FALSE)%>%
  filter(qval <0.05)%>%
  mutate(ref="Apple-banana")
DA_bluggoe <- read_tsv('Root_Treat_blugg.E_fung/significant_results.tsv', lazy = FALSE)%>%
  filter(qval <0.05)%>%
  mutate(ref="Bluggoe")
DA_hom <- read_tsv('Root_Hom.E_fung/significant_results.tsv', lazy = FALSE)%>%
  filter(qval <0.05)%>%
  mutate(ref="Hom-Thong-Mokho")
DA_plant <- read_tsv('Root_Plantain.E_fung/significant_results.tsv', lazy = FALSE)%>%
  filter(qval <0.05)%>%
  mutate(ref="Plantain")
DA_prata <- read_tsv('Root_Prata.E_fung/significant_results.tsv', lazy = FALSE)%>%
  filter(qval <0.05)%>%
  mutate(ref="Prata-Ana")
DA_lak <- read_tsv('Root_Treat_lak.E_fung/significant_results.tsv', lazy = FALSE)%>%
  filter(qval <0.05)%>%
  mutate(ref="Lakatan")
...
Cultivar_DA <- rbind(DA_arab,DA_calc4)%>%
  ...
  write_tsv('cultivar_DA_RootE_fung.tsv')

length(unique(Cultivar_DA$feature))

#_plot DAs----
Cultivar_DA <- read_tsv('cultivar_DA_RootE_fung.tsv', lazy=FALSE)
...
cult_DA_subgen <- select(Cultivar_DA,-c(metadata,stderr,pval,qval, N,stderr,`N.not.0` ))%>%
  ...
  unique()%>%
  write_tsv('cult_posDA_subgenome_fung.tsv')

#tot_tax <- mutate(tot_tax, gen_tax = paste(Phylum, Class,Order,Family,Genus, sep = "."))

cult_DA_subgen%>%
  #filter(coef>0)%>%
  ggplot(aes( x=value, fill=Order))+
  geom_bar()+
  scale_fill_adaptive(name = "nejm", palette = "default",na.value = "grey")+
  labs(fill = "Order (fungi)")+
...
ggsave("DA_BAR_O_fungy_subg.png", width = 180, height = 180, units = "mm", dpi=300)

cult_DA_subgen%>%
  ggplot(aes( x=value, fill=Class))+
  geom_bar()+
  scale_fill_adaptive(name = "d3", palette = "category20b",na.value = "grey")+
  labs(fill = "Class (fungi)")+
  facet_grid(.~`A/B`,scales = "free")+
  theme_dbox()+
  labs(#color = 'Sample',size='alr',
    x="Banana cultivar",
  ...
  theme(legend.position = "right",
        ...
        strip.text.y = element_text(size=12, angle = 0, lineheight=1),
        strip.text.x = element_text(size=12))
ggsave("DA_BAR_C_fungy_subg.png", width = 180, height = 180, units = "mm", dpi=300)
